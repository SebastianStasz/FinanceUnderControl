...
struct CashFlowFormModel: Equatable {
    ...
    var isValid: Bool {
        name.notNil && value.notNil && category.notNil
    }
    ...
    func model(for formType: CashFlowFormType<CashFlow>) -> CashFlow? {
        ...
        switch formType {
        case let .new(type):
            return CashFlow(id: UUID().uuidString, name: name, money: .init(value, currency: currency), date: date, type: type, category: category, description: description)
        case let .edit(cashFlow):
            return CashFlow(id: cashFlow.id, name: name, money: .init(value, currency: currency), date: date, type: type, category: category, description: description)
        }
    }
}
